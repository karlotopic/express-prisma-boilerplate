...
exports.loginUser = async (req, res) => {
  ...
  if (!validPassword) return res.status(401).send("Not authenticated!");
...
exports.registerUser = async (req, res) => {
  ...
  if (isExisting) return res.status(400).send("User already existing!");

  const encryptedPassword = await securityService.encryptPassword(
    password
  );
  try {
